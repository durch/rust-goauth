...
///
/// ```
pub fn get_token(jwt: &Jwt<JwtClaims>, url: Option<&str>) -> Result<Token, GOErr> {
...
pub fn get_token_as_string(jwt: &Jwt<JwtClaims>, url: Option<&str>) -> Result<String, GOErr>{
  Ok(serde_json::to_string(&get_token(jwt, url)?)?)
}
